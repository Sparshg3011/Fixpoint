...
def _redact(args: tuple[str, ...]) -> str:
    """Never let a credential reach an exception message, a log, or a UI.

    The push carries its token in a `-c http.…extraheader=AUTHORIZATION: …`
    argument. A failing git command puts its whole argv in the error — which
    is exactly how a short-lived token ends up pasted into a bug report.
    """
    safe = [re.sub(r"(extraheader=AUTHORIZATION:).*", r"\1 <redacted>", a, flags=re.I)
            for a in args]
    return " ".join(safe)
...
def _git(*args: str, cwd: Path | None = None) -> str:
    ...
    if proc.returncode != 0:
        raise RuntimeError(f"git {_redact(args)} failed:\n{proc.stderr.strip()}")
    return proc.stdout.strip()
...
def open_pr(*, upstream: str, base_commit: str, patch: str, instance_id: str,
    ...
    if app is not None:
        ...
        try:
            ...
            # Fixing a project you do not own is meant to work: you fork it and
            # install the App on the fork. Honour that here rather than making
            # the user re-run the whole fix against a different repo name.
            fork = app.installed_fork_of(upstream)
            if fork is None:
                raise PRSafetyError(str(e)) from e
            target, redirected_from = fork, upstream
        actor = app.bot_login()
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        mirror = ensure_commit(redirected_from or upstream, base_commit)
        ...
        if dry_run:
            ...
            note = f" (your fork of {redirected_from})" if redirected_from else ""
            return PRResult(url=f"(dry run) would open PR on {target}{note} as {actor}\n{stat}",
    ...
    body = _pr_body(instance_id, target, base_commit, problem_statement, resolved,
                    fixed_repo=redirected_from)
...
def _pr_body(instance_id: str, upstream: str, base_commit: str,
             problem_statement: str, resolved: bool, fixed_repo: str = "") -> str:
    ...
    origin = f" — a fork of `{fixed_repo}`, where the issue lives" if fixed_repo else ""
    return (
        f"Generated by [Fixpoint](https://github.com/Sparshg3011/Fixpoint), an autonomous "
        f"SWE-bench agent.\n\n"
        f"- **Instance**: `{instance_id}`\n"
        f"- **Opened on**: `{upstream}`{origin}\n"
